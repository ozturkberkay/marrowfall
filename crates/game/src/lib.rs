...
/// Everything a player is holding down this tick.
///
...
/// One reliable message per frame would deliver 2.4 per tick at 144 fps and
/// half of one at 30.
...
impl Input {
    ...
    /// trust boundary against a malformed frontend, not input shaping: making
    /// diagonals the same speed as cardinals is the frontend's job, because
    /// only it knows the projection the keys were pressed against.
    #[must_use]
    pub fn new(move_dir: Vec2) -> Self {
        if !move_dir.is_finite() {
            // A non-finite position spreads through every later tick and makes
            // a snapshot unequal to itself, breaking any replay comparison.
            return Self::default();
        }
        Self {
    ...
    /// Which way the player is asking to move, in tile units.
    #[must_use]
...
/// is what keeps movement independent of frame rate. Attack, dodge and jump
/// land here, where every message matters.
