...
cargo art check                           # validate the specs, measure the rigs
cargo art check --list-rules              # every gate: limit, comparison, unit, space
```

`check` reads the art on disk and prints one line per defect. Today the
committed survivor breaks seven of the thirteen `rig.*` rules, so `check`
