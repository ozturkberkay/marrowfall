...
| `crates/render` | Godot frontend (gdext). Owns the sim thread, translates input, draws snapshots. The only crate that touches Godot. |
...
- The sim runs on a **dedicated thread**, ticking at a fixed 60 Hz
  ("Fix Your Timestep" accumulator). It never touches Godot APIs — most Godot
  classes are not thread-safe, so all `Gd<T>` access stays on the main thread.
- **Boundary transports:** a crossbeam channel carries commands/input into the
  sim (every message matters); a `triple_buffer` carries `RenderSnapshot`s out
  (latest-wins — superseded snapshots are dropped, never queued).
...
godot --path project        # run the game (or open project/ in the editor)
```
...
./scripts/src/git_hooks/cargo_coverage.sh # coverage, must stay above 95%
